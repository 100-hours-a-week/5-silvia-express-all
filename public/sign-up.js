...
document.getElementById('email').addEventListener('input', function () {
  ...
  if (email === '') {
    ...
    helperText.style.display = 'block'; // 입력값이 없을 때도 helper text를 보이게 함
  } else if (!emailRegex.test(email)) {
    helperText.textContent = '*올바른 이메일 형식을 입력해주세요.(예: example@example.com)';
    helperText.style.display = 'block'; // 이메일 형식이 올바르지 않을 때 helper text를 보이게 함
...
document.getElementById('password').addEventListener('input', function () {
  password = this.value; // 비밀번호 입력란의 값을 전역 변수에 저장합니다.
  var passwordRegex =
